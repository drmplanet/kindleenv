...
static void pdfapp_showpage(pdfapp_t *app, int loadpage, int drawpage, int repaint, int transition, int searching)
{
	...
	if (repaint)
	{
		...
		if (app->shrinkwrap)
		{
			...
			PDF_LOGI("[pdfapp_showpage]===>pixwidth=%d, pixheight=%d\n", w, h);
			if (app->winw == w)
				app->panx = 0;
			if (app->winh == h)
				app->pany = 0;
...
void pdfapp_onresize(pdfapp_t *app, int w, int h)
{
	...
	PDF_LOGI("======>pdfapp_onresize::resolution=%f, pixmap = %d\n", app->resolution, fz_pixmap_width(app->ctx, app->image));
	if (app->resolution > MAXRES)
		app->resolution = MAXRES;
...
{
	int dir = 0;
	PDF_LOGI("%s: w=%d, h=%d, winw=%d, winh=%d\n", __FUNCTION__, w, h, app->winw, app->winh);
	app->winw = fz_pixmap_width(app->ctx, app->image);
	app->winh = fz_pixmap_height(app->ctx, app->image);
	if (app->winw != w || app->winh != h)
	{
		if(app->winh == h)
		{
		...
		PDF_LOGI("fun %s in 1::app->resolution=%f\n", __FUNCTION__, app->resolution);}
		else if(dir > 0)
			{app->resolution = zoom_out(app->resolution);
		PDF_LOGI("fun %s in 2::app->resolution=%f\n", __FUNCTION__, app->resolution);}
		else
			PDF_LOGI("fun %s in 3::app->resolution=%f\n", __FUNCTION__, app->resolution);
		if (app->resolution > MAXRES)
			app->resolution = MAXRES;
		if (app->resolution < MINRES)
			app->resolution = MINRES;
		PDF_LOGI("fun %s in 4::app->resolution=%f\n", __FUNCTION__, app->resolution);
		pdfapp_showpage(app, 0, 1, 1, 0, 0);
...
void pdfapp_autozoom_vertical(pdfapp_t *app)
{
	...
	PDF_LOGI("======>pdfapp_autozoom_vertical::app->winh=%d, pixmap height=%d\n", app->winh, fz_pixmap_height(app->ctx, app->image));
	PDF_LOGI("======>pdfapp_autozoom_vertical::resolution=%f\n", app->resolution);
	if (app->resolution > MAXRES)
		app->resolution = MAXRES;
	else if (app->resolution < MINRES)
		app->resolution = MINRES;
...
{
	app->resolution *= (double) app->winw / (double) fz_pixmap_width(app->ctx, app->image);
	PDF_LOGI("======>pdfapp_autozoom_horizontal::resolution=%f\n", app->resolution);
	if (app->resolution > MAXRES)
		app->resolution = MAXRES;
	else if (app->resolution < MINRES)
		app->resolution = MINRES;
...
void pdfapp_autozoom(pdfapp_t *app)
{
	...
	PDF_LOGI("[%s]===>page_aspect=%f, win_aspect=%f\n", __FUNCTION__, page_aspect, win_aspect);
	PDF_LOGI("[%s]===>pixmap_width=%d, pixmap_height=%d\n", __FUNCTION__, fz_pixmap_width(app->ctx, app->image), fz_pixmap_height(app->ctx, app->image));
	PDF_LOGI("[%s]===>win_width=%d, win_height=%d\n", __FUNCTION__, app->winw, app->winh);
...
void pdfapp_autozoom(pdfapp_t *app)
{
	...
	w = fz_pixmap_width(app->ctx, app->image);
	h = fz_pixmap_height(app->ctx, app->image);
	PDF_LOGI("%s: w=%d, h=%d, winw=%d, winh=%d\n", __FUNCTION__, w, h, app->winw, app->winh);
	...
	if (app->winw != w || app->winh != h)
	{
		...
		app->winw = w*(app->resolution/72.0f);
		app->winh = h*(app->resolution/72.0f);
		pdfapp_showpage(app, 0, 1, 1, 0, 0);
	}
}
#endif
...
void pdfapp_fini(pdfapp_t *app)
{
	return;
}

int pdfapp_ext_set_presentation_mode(pdfapp_t *app, int nValue)
{
	app->presentation_mode = nValue;
	return 0;
}

int pdfapp_ext_get_presentation_mode(pdfapp_t *app)
{
	return app->presentation_mode;
}

int pdfapp_ext_set_shrinkwrap(pdfapp_t *app, int nValue)
{
	app->shrinkwrap = nValue;
	return 0;
}

int pdfapp_ext_get_shrinkwrap(pdfapp_t *app)
{
	return app->shrinkwrap;
}

int pdfapp_ext_set_screenwh(pdfapp_t *app, int nWide, int nHeight)
{
	app->scrw = nWide;
	app->scrh = nHeight;
	return 0;
...
static void rect_union(MyRectangle r)
{
	if(finalRect.Left > r.Left)
		finalRect.Left = r.Left;
	else if(finalRect.Top > r.Top)
		finalRect.Top = r.Top;
	else if(finalRect.Right < r.Right)
		finalRect.Right = r.Right;
	else if(finalRect.Bottom <  r.Bottom)
		finalRect.Bottom = r.Bottom;
	PDF_LOGI("[%s](%d, %d, %d, %d)", __FUNCTION__, finalRect.Left, finalRect.Top, finalRect.Right, finalRect.Bottom);
}
void pdfapp_copytext(pdfapp_t *app, MyRectangle myrect, int *ucsbuf, int *insize, MyRectangle* outrect)
{
	fz_rect hitbox;
	fz_matrix ctm;
	fz_text_page *page = app->page_text;
	...
	float xscale = (double) app->winw / (double) fz_pixmap_width(app->ctx, app->image);
	float yscale = (double) app->winh / (double) fz_pixmap_height(app->ctx, app->image);

	float x0 = myrect.Left/xscale;
	float x1 = myrect.Right/xscale;
	float y0 = myrect.Top/yscale;
	float y1 = myrect.Bottom/yscale;
	/*PDF_LOGI("[%s]left=%d, right=%d, top=%d, bottom=%d\n", __FUNCTION__, 
		myrect.Left, myrect.Right, myrect.Top, myrect.Bottom);*/
	//x0 = 10;
	//y0 = 20;
	//x1 = 150;
	//y1 = 150;
	PDF_LOGI("[%s]left=%f, right=%f, top=%f, bottom=%f\n", __FUNCTION__, x0, x1, y0, y1);
		...
		for (line = block->lines; line < block->lines + block->len; line++)
		{
			...
			for (span = line->first_span; span; span = span->next)
			{
				for (i = 0; i < span->len; i++)
				{
					...
					if (c < 32)
						c = '?';
					PDF_LOGI("character size:[%f, %f]\n", (hitbox.x1 - hitbox.x0), (hitbox.y1 - hitbox.y0));
					if (hitbox.x1 >= x0 && hitbox.x0 <= x1 && hitbox.y1 >= y0 && hitbox.y0 <= y1)
					{
						saw_text = 1;
						...
						{
							ucsbuf[p++] = c;
							PDF_LOGI("[%s]-----L=%d, the charactor = %d\n", __FUNCTION__, __LINE__,c);
							//add in 2016.03.01
							temp.Left	= hitbox.x0*xscale;
							temp.Right	= hitbox.x1*xscale;
							temp.Top		= hitbox.y0*yscale;
							temp.Bottom 	= hitbox.y1*yscale;
							rect_union(temp);
						}
					}
				}
			}
			PDF_LOGI("[%s]-----L=%d, the total character size = %d\n", __FUNCTION__, __LINE__, p);
			if (saw_text)
				need_newline = 1;
		}
	}
...
#if 0
void pdfapp_getlinepos(pdfapp_t *app, int start_x, int start_y, MyRectangle* startrect, MyRectangle* endrect)
{
	fz_rect hitbox;
	fz_matrix ctm;
	fz_text_page *page = app->page_text;
	...
	float x0 = start_x/xscale;
	float y0 = start_y/yscale;
	PDF_LOGI("[%s]====>xscale=%f, yscale=%f\n", __FUNCTION__, xscale, yscale);
	PDF_LOGI("[%s]====>winw=%d, winh=%d, pix_width=%d, pix_height=%d\n", __FUNCTION__, app->winw, app->winh, 
		fz_pixmap_width(app->ctx, app->image), fz_pixmap_height(app->ctx, app->image));
	PDF_LOGI("[%s]start_x=%d, start_y=%d, x0=%f, y0=%f\n", __FUNCTION__, start_x, start_y, x0, y0);
	
	pdfapp_viewctm(&ctm, papp);
		...
		for (line = block->lines; line < block->lines + block->len; line++)
		{
			...
			for (span = line->first_span; span; span = span->next)
			{
				for (i = 0; i < span->len; i++)
				{
					fz_text_char_bbox(app->ctx, &hitbox, span, i);
					fz_transform_rect(&hitbox, &ctm);
					//PDF_LOGI("[%s]x0=%f, y0=%f, x1=%f, y1=%f\n", __FUNCTION__, hitbox.x0, hitbox.y0,
							//hitbox.x1, hitbox.y1);
					if (hitbox.x0 <= x0  && hitbox.y0 <= y0 && hitbox.x1 >= x0 && hitbox.y1 >= y0)
					{
						//return the start postion of one line
						startrect->Left = hitbox.x0*xscale;
						startrect->Top = hitbox.y0*yscale;
						startrect->Right = hitbox.x1*xscale;
						startrect->Bottom = hitbox.y1*yscale;

						//debug
						fz_text_char_bbox(app->ctx, &hitbox, span, span->len -1);
						fz_transform_rect(&hitbox, &ctm);
						PDF_LOGI("[%s]x0=%f, y0=%f, x1=%f, y1=%f\n", __FUNCTION__, hitbox.x0, hitbox.y0,
							hitbox.x1, hitbox.y1);
						flag = 1;
				...
				if(flag == 1)
				{
					//return the last postion of one line
					fz_text_char_bbox(app->ctx, &hitbox, span, span->len -1);
					fz_transform_rect(&hitbox, &ctm);
					endrect->Left = hitbox.x0*xscale;
					endrect->Top = hitbox.y0*yscale;
					endrect->Right = hitbox.x1*xscale;
					endrect->Bottom = hitbox.y1*yscale;
					PDF_LOGI("[%s]left=%d, top=%d, right=%d, bottom=%d\n", __FUNCTION__, endrect->Left, endrect->Top,
		...
		for (line = block->lines; line < block->lines + block->len; line++)
		{
			...
			for (span = line->first_span; span; span = span->next)
			{
				for (i = 0; i < span->len; i++)
				{
					...
					{
						//return the start postion of one line
						startrect->Left = hitbox.x0;
						startrect->Top = hitbox.y0;
						startrect->Right = hitbox.x1;
						startrect->Bottom = hitbox.y1;
				...
				if(flag == 1)
				{
					...
					fz_text_char_bbox(app->ctx, &hitbox, span, span->len -1);
					fz_transform_rect(&hitbox, &ctm);
					endrect->Left = hitbox.x0;
					endrect->Top = hitbox.y0;
					endrect->Right = hitbox.x1;
					endrect->Bottom = hitbox.y1;
					PDF_LOGI("[%s]left=%d, top=%d, right=%d, bottom=%d\n", __FUNCTION__, endrect->Left, endrect->Top,
						endrect->Right, endrect->Bottom);
				}
			}
